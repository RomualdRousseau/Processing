...
class Bone {
  ...
  private void ensureTipVectorUpdated() {
    if (this.tipChanged) {
      ...
      PMatrix rx = new PMatrix(3, 3); 
      rx.rotateX(this.gloAngle[0]);
      PMatrix ry = new PMatrix(3, 3); 
      ry.rotateY(this.gloAngle[1]);
      PMatrix rz = new PMatrix(3, 3); 
      rz.rotateZ(this.gloAngle[2]);
